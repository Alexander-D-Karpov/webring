...
// A link the server never sent is one a script added, which a reader with scripts off
// will never see.
func TestMarkRawLinksSeparatesServedFromScripted(t *testing.T) {
	links := []Link{
		{Href: "https://otor.ing/mine/next"},
		{Href: "https://otor.ing/mine/prev"},
	}
	markRawLinks(links, `<a href="https://otor.ing/mine/next">next</a>`)

	if !links[0].InRawHTML {
		t.Errorf("a link present in the markup was called scripted")
	}
	if links[1].InRawHTML {
		t.Errorf("a link absent from the markup was called served")
	}
}

// Without a plain fetch to compare against there is no evidence either way, so the site
// gets the benefit of the doubt rather than an accusation.
func TestMarkRawLinksAssumesServedWhenTheFetchFailed(t *testing.T) {
	links := []Link{{Href: "https://otor.ing/mine/next"}}
	markRawLinks(links, "")

	if !links[0].InRawHTML {
		t.Errorf("a failed plain fetch was treated as proof the link needs scripts")
	}
}

func TestMarkRawLinksMatchesAcrossSchemes(t *testing.T) {
	links := []Link{{Href: "https://otor.ing/mine/next"}}
	markRawLinks(links, `<a href="//otor.ing/mine/next">next</a>`)

	if !links[0].InRawHTML {
		t.Errorf("a scheme-relative link was not matched")
	}
}

// Not every member uses href. One drives its widget from an onclick handler, and that
// link works perfectly well for a visitor, so it has to count.
func TestVisitFollowsAnchorsWiredThroughOnclick(t *testing.T) {
	browser := testBrowser(t)
	url := servePage(t, `<html><body><p>my site</p>
		<a title="Previous" onclick="location.href='https://otor.ing/mine/prev'">&lt;</a>
		<a title="Next" onclick="location.href='https://otor.ing/mine/next'">&gt;</a>
	</body></html>`)

	obs := visit(t, browser, url)

	link := linkFor(t, obs, "/mine/next")
	if !link.Visible {
		t.Errorf("onclick-driven link = %+v, want visible", link)
	}
	if !link.aboveFold(ViewportDesktop) {
		t.Errorf("onclick-driven link at the top of the page is not above the fold")
	}
}

func TestVisitResolvesRelativeOnclickTargets(t *testing.T) {
	browser := testBrowser(t)
	url := servePage(t, `<html><body><p>x</p>
		<a onclick="location.href='/some/page'">go</a>
	</body></html>`)

	link := linkFor(t, visit(t, browser, url), "/some/page")
	if !strings.HasPrefix(link.Href, "http://") {
		t.Errorf("relative onclick target was not resolved: %q", link.Href)
	}
}

// An onclick that carries no URL is just a script, not a link.
func TestVisitIgnoresAnchorsWithNoDestination(t *testing.T) {
	browser := testBrowser(t)
	url := servePage(t, `<html><body><p>x</p>
		<a onclick="dialog.showModal()">open</a>
		<a>no destination at all</a>
	</body></html>`)

	obs := visit(t, browser, url)
	if len(obs.Links) != 0 {
		t.Errorf("collected %v, want nothing", hrefs(obs))
	}
}
